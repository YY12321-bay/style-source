...
| # | 项目 | 说明 | 工作量 | 难度 |
|---|------|------|--------|------|
| **P0-1** | **图片 `loading="lazy"` 属性** | `gallery-runtime.js` 中动态生成 img 时加 `loading="lazy"`，当前 `setupLazyLoading()` 是空函数！ | 小 | ★☆☆ |
| **P0-2** | **JS 异步加载** | `gallery-runtime.js` 改为 `<script defer>` 或 `<script type="module">` | 小 | ★☆☆ |
| **P0-3** | **修复 tags 重复问题** | 当前 880 个标签中大量同义标签（"3D"/"3D卡通"/"3D角色"、"插画"/"手绘"等），建议统一 | 中 | ★★☆ |
| **P0-4** | **清理旧版 styles/gallery.html** | 删除 `styles/gallery.html` 旧版残留，避免混淆 | 小 | ★☆☆ |
...
| **P1-2** | **渐进式图片加载** | 先展示缩略图 `blur-up` 过渡，再加载全尺寸 | 中 | ★★☆ |
| **P1-3** | **一键复制提示词** | Lightbox 中增加"复制完整提示词"按钮，含变量替换功能 | 中 | ★★☆ |
| **P1-4** | **标签云重新设计** | 当前 880 标签太多，建议：
  - 设计层级标签结构（中/英标签对齐）
  - 提供标签搜索/过滤
  - 高频标签优先展示 | 大 | ★★★ |
...
| **P2-2** | **统计/分析** | Umami / Plausible 自托管，了解用户行为 | 小 | ★☆☆ |
| **P2-3** | **随机发现按钮** | 在搜索栏附近加"🎲 随机"按钮，随机展示一个风格 | 小 | ★☆☆ |
| **P2-4** | **分享卡片功能** | 分享到 X/Twitter/微信等 | 中 | ★★☆ |
...
| **P3-6** | 缓存策略优化 | JS/CSS 长期缓存 + CDN 缓存头 | 小 | ★☆☆ |
